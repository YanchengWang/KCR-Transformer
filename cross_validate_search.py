...
group.add_argument('--model-search', action='store_true', default=False,
                   help='Enable model search by loading the supernet')
group.add_argument('--cost-weight', type=float, default=0.2,
                   help='weight of computation cost loss')
group.add_argument('--cost-weight-cvlist', type=float, nargs=9, default=[0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5],
                   help='list of weight of computation cost loss for cv')
...
def train_one_epoch_search(
    ...
    losses_cost_m = utils.AverageMeter()
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            output, comput_cost = model(input)
            loss_CE = loss_fn(output, target)

            loss = loss_CE + args.cost_weight * comput_cost

        if not args.distributed:
            losses_m.update(loss.item(), input.size(0))
            losses_cost_m.update(comput_cost.item(), input.size(0))
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if args.distributed:
                ...
                reduced_loss_ib = utils.reduce_tensor(comput_cost.data, args.world_size)
                losses_cost_m.update(reduced_loss_ib.item(), input.size(0))
            ...
            if utils.is_primary(args):
                _logger.info(
                    ...
                    'Comput Cost: {ib_loss.val:#.4g} ({ib_loss.avg:#.3g})  '
...
def run_cv(args, dataset_train_splits, device):
    for cost_weight in args.cost_weight_cvlist:
        args.cost_weight = cost_weight
        ...
        for i in range(args.cvfold_number):
            ...
            try:
                for epoch in range(start_epoch, num_epochs):
                    ...
                    train_metrics = train_one_epoch_search(
                        epoch,
                        model,
                        loader_train,
                        optimizer,
